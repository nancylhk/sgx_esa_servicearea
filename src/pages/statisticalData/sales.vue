	...
	<div class="app-container">
		...
		<div class="app-main" id="app-main">
			...
			<div v-show="total > 0" class="dataSummary">
				...
				<span>总销售额:{{saleTotalInfo.totalSalesVolume}}元</span>
	...
	export default {
			...
			timeRange: {
				handler: function(newVal, oldVa) {
					...
					if(newVal==null || newVal==''|| newVal==undefined){
						...
						self.beginTime = self.timeRange.length > 0 ? self.timeRange[0] : '';
		...
		methods: {
			...
			exportReport() {
				...
				var url = "/report/exportSaleReport?accessToken=" + self.$store.state.user.token + "&restAreaId=" + self.restAreaId + "&shopId=" + self.shopId + "&beginTime=" + self.beginTime + "&endTime=" + self.endTime;
				a.href = url;
				a.click();
			...
			getList() {
				...
				this.$http.get(self.api.getSaleReport, {
					params: {
						accessToken: self.$store.state.user.token,
					...
					if(response.status == 200) {
						self.saleReportList = response.data; //saleReportList
					}
				}, function(response) {})
			...
			getListCount() {
				let self = this;
				self.$http.get(self.api.getSaleReportTotalInfo, {
					params: {
						accessToken: self.$store.state.user.token,
						restAreaId: self.restAreaId,
						shopId: self.shopId,
