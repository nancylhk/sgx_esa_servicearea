			...
			<div class="mt30">
				...
				<el-form-item label="所属公用库：" prop="publicStorageID" class="inbk">
					<el-select v-model="addInfo.publicStorageID">
						<el-option v-for="(item,index) in storageList" :key="item.id" :label="item.commonName" :value="item.id">
						</el-option>
					</el-select>
				</el-form-item>
				...
				<el-form-item label="等级：" prop="commodityLevel" class="form-right">
					<el-select v-model="addInfo.commodityLevel">
						<el-option v-for="(item,index) in commodityLevels" :key="item.levelCode" :label="item.levelName" :value="item.levelCode">
						</el-option>
					</el-select>
				</el-form-item>
				...
				<el-form-item label="规格" required class="re-el-item inbk">
					...
					<el-form-item prop="specification" class="inbk">
						<el-select v-model="addInfo.specification">
							<el-option v-for="(item,index) in specifications" :key="item.specificationCode" :label="item.specificationName" :value="item.specificationCode">
							</el-option>
						</el-select>
					</el-form-item>
				</el-form-item>
				...
				<el-form-item label="保质期" required class="re-el-item form-right">
					...
					<el-form-item prop="guaranteePeriod" class="inbk">
						<el-select v-model="addInfo.guaranteePeriod">
							<el-option v-for="(item,index) in guaranteePeriods" :key="item.periodCode" :label="item.periodName" :value="item.periodCode">
							</el-option>
						</el-select>
					</el-form-item>
				</el-form-item>
			...
			<div class="mt30">
				...
				<el-form-item label="计价单位：" prop="priceUnit" class="form-right">
					<el-select v-model="addInfo.priceUnit">
						<el-option v-for="(item,index) in priceUnits" :key="item.priceUnitID" :label="item.priceUnitName" :value="item.priceUnitID">
						</el-option>
					</el-select>
				</el-form-item>
				...
				<el-form-item label="店铺类型：" prop="shopTypeID" class="inbk">
					<el-select v-model="addInfo.shopTypeID">
						<el-option v-for="(item,index) in shopTypes" :key="item.shopTypeCode" :label="item.shopTypeName" :value="item.shopTypeCode">
						</el-option>
					</el-select>
				</el-form-item>
				<el-form-item label="出售类型：" prop="commodityType"  class="form-right">
	...
	export default {
		...
		created() {
			...
			this.$http.get(this.api.getShopType, {
				...
				self.shopTypes = response.data.shopTypes;
			}, function(response) {})
				...
				this.$refs[formName].validate((valid) => {
					if(valid) {
						self.$http.get(self.api.addPublicStorageProduct, {
							...
							//	self.startProcess(response.data);
