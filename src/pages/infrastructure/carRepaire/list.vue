	...
	<div>
		...
		<div class="app-main" id="app-main">
			<table>
				...
				<tbody>
					<tr v-for="(info,index) in carRepaireList">
						<td>{{index+1}}</td>
						...
						<td>{{info.measure}}</td>
						<td>
							<a @click="viewEvent(info.serviceID)">查看</a>
							<a @click="updateEvent(info.serviceID)">修改</a>
							<a @click="deleteEvent(info.serviceID)">删除</a>
						</td>
					</tr>
				</tbody>
			</table>
		...
		methods: {
			...
			deleteEvent(ID) {
				this.$confirm('确认删除？', '提示', {
					...
					var self = this;
					self.$http.get(self.api.deleteCarRepaireInfo, {
						params: {
							accessToken: self.$store.state.user.token,
							serviceID: ID,
						}
			...
			getList() {
				...
				this.$http.get(this.api.getCarRepaireList, {
					params: {
					...
					if(response.status == 200) {
						self.carRepaireList = response.data;
					}
				}, function(response) {})
			...
			getListCount(){
				let self = this;
				self.$http.get(self.api.getCarRepaireListCount, {
					params: {
						accessToken: self.$store.state.user.token,
						businessType: self.businessType,
						serviceName:self.serviceName,
