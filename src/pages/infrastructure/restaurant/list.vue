	...
	<div>
		<div class="app-search ml10 mt5">
			<el-form :inline="true" class="demo-form-inline">
				<el-form-item label="名称">
					<el-input v-model="restaurantName"></el-input>
				</el-form-item>
		...
		<div class="app-main" id="app-main">
			<table>
				...
				<tbody>
					<tr v-for="(info,index) in restaurantList">
						<td>{{index+1}}</td>
						<td>{{info.restaurantName}}</td>
						<td>{{info.distribution}}</td>
						<td>{{info.businessType}}</td>
						<td>
							<a @click="viewEvent(info.restaurantID)">查看</a>
							<a @click="updateEvent(info.restaurantID)">修改</a>
							<a @click="deleteEvent(index,info.restaurantID)">删除</a>
						</td>
					</tr>
				</tbody>
			</table>
		...
		methods: {
			...
			deleteEvent(idx, ID) {
				this.$confirm('确认删除？', '提示', {
					...
					var self = this;
					self.$http.get(self.api.deleteRestaurantInfo, {
						params: {
							accessToken: self.$store.state.user.token,
							restaurantID: ID,
						}
			...
			getList() {
				...
				self.$http.get(self.api.getRestaurantList, {
					params: {
						accessToken: self.$store.state.user.token,
			...
			getListCount() {
				let self = this;
				self.$http.get(self.api.getRestaurantCount, {
					params: {
						accessToken: self.$store.state.user.token,
						restaurantName: self.restaurantName,
						businessType: self.businessType,
