	...
	<div>
		...
		<div class="app-main"  id="app-main">
			<table>
				...
				<tbody>
					<tr v-for="(info,index) in carParkList">
						<td>{{index+1}}</td>
						<td>{{info.parkingName}}</td>
						<th>{{info.businessType}}</th>
						<td>{{info.distribution}}</td>
						<td>{{info.measure}}</td>
						<td>
							<a @click ="viewEvent(info.parkingID)">查看</a>
							<a @click ="updateEvent(info.parkingID)">修改</a>
							<a @click ="deleteEvent(info.parkingID)">删除</a>
						</td>
					</tr>
				</tbody>
			</table>
		...
		methods: {
			...
			deleteEvent(ID) {
				this.$confirm('确认删除？', '提示', {
					...
					var self = this;
					self.$http.get(self.api.deleteCarParkInfo, {
						params: {
							accessToken: self.$store.state.user.token,
							parkingID: ID,
						}
			...
			getList() {
				...
				this.$http.get(this.api.getCarParkList, {
					params: {
						accessToken: this.$store.state.user.token,
						parkingName: self.parkingName,
					...
					if(response.status == 200) {
						self.carParkList = response.data;
			...
			getListCount(){
				let self = this;
				self.$http.get(self.api.getParkingLotListCount, {
					params: {
						accessToken: self.$store.state.user.token,
						parkingName: self.parkingName,
						businessType: self.businessType,
